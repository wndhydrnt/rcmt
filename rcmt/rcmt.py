...
def can_merge_after(
    ...
    passed = datetime.datetime.now(tz=datetime.UTC) - created_at
    return passed >= delay
...
def execute(opts: Options) -> bool:
    ...
    if len(opts.repositories) > 0:
        ...
        if needs_all_repositories is True or execution.executed_at is None:
            since = datetime.datetime.fromtimestamp(0, tz=datetime.UTC)
        else:
    ...
    ex.executed_at = datetime.datetime.now(tz=datetime.UTC)
