...
def execute(directory: str, opts: Options, repo_name: str) -> None:
    ...
    for task_path in opts.task_paths:
        ...
        if result is False:
            print(f"❌ - at least one Matcher did not match repository {repo_name}")
            return
        ...
        if gitc.has_changes(repo_dir=checkout_dir):
            print(f"😍 Actions modified files - view changes in {checkout_dir}")
        else:
            print("🤔 No changes after applying Actions")
